...
**Context:** Config with govet, errcheck, staticcheck, gosimple, unused, ineffassign, gocritic, misspell. Targeted suppressions for Bubble Tea patterns (hugeParam) and SDK idioms. All existing lint issues fixed or suppressed with rationale.

**Completed:** v0.3.2.0 (2026-03-28)
...
**Context:** Already implemented: bash (classifies commands as read-only/DML/destructive) and bigquery_query (SQL classification + cost-aware gating). GCS is read-only and doesn't need it. No remaining tools require migration.

**Completed:** v0.3.2.0 (2026-03-28)

### GCS Binary Detection Magic Bytes

**What:** Use file magic bytes to detect binary files before attempting to read GCS objects.

**Context:** `sniffBinary()` checks 18 signatures (Parquet, Avro, ORC, gzip, Snappy, Zstandard, PNG, JPEG, ZIP/DOCX, PDF, GIF, ELF, Mach-O 32/64 + swapped) plus a 30% non-printable heuristic.
