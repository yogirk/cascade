...
- **DuckDB as a first-class engine.** Three new tools: `duckdb_query` (SQL against any `.duckdb` file and/or `gs://*.parquet` via httpfs), `duckdb_schema` (list/describe/sample), and `bq_to_duckdb` (pull a BigQuery slice into a local table). The wedge: when a user is iterating, they can pull a slice once and beat on it locally with no per-scan cost — and bring their own `.duckdb` files into the conversation without staging through BQ at all.
- **`bq_to_duckdb` modes.** `mode=local` streams BQ rows to a temp CSV and creates the table — zero config, capped at ~1 GiB. `mode=gcs` runs BQ EXPORT to a staging bucket, then COPYs Parquet shards via httpfs — capped at ~50 GiB, requires `[duckdb] staging_bucket`. `mode=auto` (default) picks based on size and config; refuses with a redirect when the choice doesn't fit (e.g., requesting `local` on a 100 GiB slice points the user at `gcs`).
- **Bring-your-own-`.duckdb`.** Both `duckdb_query` and `duckdb_schema` accept an optional `database='/path/to/your.duckdb'` field that retargets the call from the per-session DB to a user-owned file. Cascade does not touch the file's lifecycle and skips the session RWMutex when operating on external paths.
...
- **Volume gate** on `bq_to_duckdb`. Three thresholds: `warn_bytes` (default 1 GiB, informational), `hard_stop_bytes` (default 50 GiB, GCS path), `local_hard_stop_bytes` (default 1 GiB, local-stream path). `force=true` bypasses the hard stop. Asymmetric on purpose — pulling terabytes onto a laptop is real damage, while reading more than expected from local DuckDB self-corrects.
